...
test.describe("メールアドレス認証ページ（/login/verify-email）のテスト", () => {
  test("トークンなしの場合に無効なURLメッセージが表示される", async ({
    page,
  }) => {
    // トークンなしでアクセス
  ...
  test("トークンなしの場合が無効なURLメッセージが表示される", async ({
    ...
    // トークンはあるがメールなし
    await page.goto("/login/verify-email?email=test@example.com");
...
test.describe("メール認証エラーケースのテスト（verifyEmailAction参照）", () => {
  test("verificationTokenテーブルからトークンが取得できなかった場合のエラーが表示される", async ({
    page,
  }) => {
    // 存在しないトークンでアクセス
    const nonexistentToken = "invalid_token_" + Date.now();
    await page.goto(
      `/login/verify-email?token=${nonexistentToken}&email=test@example.com`,
    );
    ...
    // エラーメッセージが表示される
    // verifyEmailAction実装: トークンが見つからない場合は「リンクの有効期限が切れている」と扱う
    const errorMessage = page.getByText(/リンクの有効期限が切れています/);
    await expect(errorMessage).toBeVisible();

    // 「アカウント作成へ」と「ログインへ」のリンクが表示されている
    const signupLink = page.getByRole("link", { name: "アカウント作成へ" });
  ...
  test("verificationTokenテーブルからトークンが取得できたが現在の時刻がexpiresを超えていた場合のエラーが表示される", async ({
    page,
  }) => {
    // 期限切れトークンを作成
    const testEmail = `expired-token-test-${Date.now()}@example.com`;
    const { token, email } = await createExpiredToken(testEmail, 1); // 1時間前に期限切れ

    // 期限切れトークンでアクセス
    await page.goto(
      `/login/verify-email?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`,
    );

    // エラーメッセージが表示される
    // verifyEmailAction実装: 期限切れの場合のエラーメッセージ
    const errorHeading = page.getByRole("heading", {
      name: "認証に失敗しました",
    });
    await expect(errorHeading).toBeVisible();
    ...
    // 「アカウント作成へ」リンクが表示されている
    const signupLink = page.getByRole("link", { name: "アカウント作成へ" });
    await expect(signupLink).toBeVisible();

    // テスト後のクリーンアップ
    await deleteVerificationTokensByEmail(testEmail);
  });

  test("userテーブルからユーザーが取得できなかった場合のエラーが表示される", async ({
    page,
  }) => {
    // ユーザーなしの有効なトークンを作成
    const testEmail = `nonexistent-user-${Date.now()}@example.com`;
    const { token, email } = await createTokenWithoutUser(testEmail);
    ...
    // エラーメッセージが表示される
    // verifyEmailAction実装: ユーザーが見つからない場合のエラーメッセージ
    const errorHeading = page.getByRole("heading", {
      name: "認証に失敗しました",
    });
    await expect(errorHeading).toBeVisible();

    const errorMessage = page.getByText(/ユーザーが見つかりません/);
    await expect(errorMessage).toBeVisible();
